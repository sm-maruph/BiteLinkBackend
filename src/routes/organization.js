...
export async function organizationRoutes(app){
  app.post('/restaurants',async(request,reply)=>withTransaction(request.context,async client=>{
    ...
    const {rows}=await client.query("insert into app.restaurants(tenant_id,name,slug,status) values($1,$2,$3,'draft') returning *",[request.context.tenantId,body.name,body.slug])
  ...
  app.post('/outlets',async(request,reply)=>withTransaction(request.context,async client=>{
    ...
    if(!access.rows[0]?.allowed||!subscription.rows[0]||!['trialing','active'].includes(subscription.rows[0].status))return reply.code(402).send({error:'active_subscription_required'})
