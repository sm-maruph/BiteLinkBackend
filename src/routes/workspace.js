...
import { createTableToken } from '../modules/guest/table-token.js'
...
export async function workspaceRoutes(app) {
  app.post('/tables', async (request, reply) => withTransaction(request.context, async (client) => {
    ...
    const created=await client.query(`insert into app.dining_tables(tenant_id,restaurant_id,outlet_id,table_number,qr_token_hash,capacity,status)
      values($1,$2,$3,$4,$5,$6,'available') returning id,table_number,capacity,status,qr_token_hash`,[request.context.tenantId,restaurantId,outletId,normalizedNumber,qrHash,normalizedCapacity])
    return reply.code(201).send({...created.rows[0],qr_token:createTableToken(created.rows[0]),qr_token_hash:undefined})
  }))
  ...
  app.post('/tables/:id/rotate-qr',async(request,reply)=>withTransaction(request.context,async client=>{const id=request.params.id,row=await client.query('select restaurant_id,outlet_id from app.dining_tables where tenant_id=$1 and id=$2',[request.context.tenantId,id]);if(!row.rows[0])return reply.code(404).send({error:'table_not_found'});const access=await client.query("select app.has_permission($1,'tables.write',$2,$3) allowed",[request.context.tenantId,row.rows[0].restaurant_id,row.rows[0].outlet_id]);if(!access.rows[0]?.allowed)return reply.code(403).send({error:'permission_denied'});const qrHash=createHash('sha256').update(randomBytes(32)).digest('hex'),updated=await client.query('update app.dining_tables set qr_token_hash=$3,updated_at=now() where tenant_id=$1 and id=$2 returning id,table_number,capacity,status,qr_token_hash',[request.context.tenantId,id,qrHash]);return {...updated.rows[0],qr_token:createTableToken(updated.rows[0]),qr_token_hash:undefined}}))
  ...
  app.get('/workspace', async (request, reply) => withTransaction(request.context, async (client) => {
    ...
    const tables = await client.query('select id,table_number,capacity,status,qr_token_hash from app.dining_tables where tenant_id=$1 and restaurant_id=$2 and outlet_id=$3 order by table_number',[request.context.tenantId,restaurantId,outletId])
    const orders = await client.query(`select o.*,t.table_number,rv.rating review_rating,rv.comment review_comment,coalesce(string_agg(oi.item_name_snapshot||' x '||oi.quantity,'; ' order by oi.created_at),'') items
      ...
      where o.tenant_id=$1 and o.restaurant_id=$2 and o.outlet_id=$3 group by o.id,t.table_number,rv.rating,rv.comment order by o.placed_at desc limit 200`,[request.context.tenantId,restaurantId,outletId])
    const payments = await client.query(`select p.*,t.table_number from app.payments p join app.table_sessions s on s.id=p.session_id and s.tenant_id=p.tenant_id
      join app.dining_tables t on t.id=s.table_id and t.tenant_id=s.tenant_id where p.tenant_id=$1 and p.restaurant_id=$2 and p.outlet_id=$3 order by p.created_at desc limit 200`,[request.context.tenantId,restaurantId,outletId])
    const requests = await client.query(`select q.*,t.table_number from app.service_requests q join app.dining_tables t on t.id=q.table_id and t.tenant_id=q.tenant_id
      where q.tenant_id=$1 and q.restaurant_id=$2 and q.outlet_id=$3 order by q.created_at desc limit 200`,[request.context.tenantId,restaurantId,outletId])
    ...
    const qrAccess=await client.query("select app.has_permission($1,'tables.write',$2,$3) allowed",[request.context.tenantId,restaurantId,outletId])
    const safeTables=tables.rows.map(table=>({...table,qr_token:qrAccess.rows[0]?.allowed?createTableToken(table):undefined,qr_token_hash:undefined}))
    return {restaurant:restaurant.rows[0],menu:menu.rows,offers:offers.rows,tables:safeTables,orders:orders.rows,payments:payments.rows,requests:requests.rows,team:team.rows,
  ...
  app.get('/restaurant-analytics', async (request, reply) => withTransaction(request.context, async (client) => {
    ...
    const analyticsAccess=await client.query("select app.has_permission($1,'analytics.read',$2,null) allowed",[request.context.tenantId,restaurantId])
    if(!analyticsAccess.rows[0]?.allowed) return reply.code(403).send({error:'permission_denied'})
    ...
    const orders=await client.query("select outlet_id,status,grand_total,placed_at from app.orders where tenant_id=$1 and restaurant_id=$2 and placed_at>=now()-interval '90 days'",[request.context.tenantId,restaurantId])
    ...
    const payments=await client.query("select status,amount from app.payments where tenant_id=$1 and restaurant_id=$2 and created_at>=now()-interval '90 days'",[request.context.tenantId,restaurantId])
