...
export const menuCategoryBody=z.object({restaurantId:uuid,name:z.string().trim().min(2).max(160),slug:z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).max(80),description:z.string().trim().max(500).optional()})
export const menuItemBody=z.object({restaurantId:uuid,categoryId:uuid,name:z.string().trim().min(2).max(160),slug:z.string().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).max(80),description:z.string().trim().max(1000).optional(),imageUrl:z.string().url().optional().or(z.literal('')),basePrice:z.coerce.number().min(0).max(10000000),preparationMinutes:z.coerce.number().int().min(0).max(1440).default(20),featured:z.boolean().default(false)})
